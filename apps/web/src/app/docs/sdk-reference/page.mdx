...
const earnkit = new EarnKit({
  ...
  baseUrl: 'https://your-api.com', // Optional: defaults to localhost:3000
...
interface EarnKitConfig {
  ...
  requestTimeoutMs?: number; // Optional: Request timeout (default: 30000)
}
```
...
try {
  ...
  if (error instanceof EarnKitApiError && error.status === 0) {
    ...
    // The SDK has built-in retry logic, but you can add your own
  }
}
```
...
- **Automatic Retries**: Up to 2 retries for server errors (5xx) and timeouts
